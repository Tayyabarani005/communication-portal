<x-app-layout>
    ...
    <div class="flex-1 flex items-center justify-center p-8" style="background: var(--color-bg-main);">
        <div class="w-full max-w-lg rounded-2xl shadow-lg border p-8" style="background: white; border-color: var(--color-border);">
            ...
            <form method="POST" action="{{ route('workspaces.channels.store', $workspace) }}" class="space-y-5" x-data="{ isPrivate: '{{ old('is_private', '0') }}' }">
                @csrf
                ...
                <div>
                    ...
                    <div class="flex gap-3">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="is_private" value="0" x-model="isPrivate" class="rounded" id="vis_public">
                            <span class="text-sm" style="color: var(--color-primary-700);"># Public</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="is_private" value="1" x-model="isPrivate" class="rounded" id="vis_private">
                ...
                <div id="member-select-section" x-show="isPrivate === '1'" x-cloak>
                    ...
                    <div class="max-h-48 overflow-y-auto rounded-xl border p-2 space-y-1" style="border-color: var(--color-border);">
                        @php
                            $availableMembers = $members->reject(fn ($member) => $member->user_id === auth()->user()->user_id);
                        @endphp
                        ...
                        @foreach($availableMembers as $m)
                            <label class="flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                                <input type="checkbox" name="members[]" value="{{ $m->user_id }}" class="rounded" style="accent-color: var(--color-accent-600);" {{ in_array($m->user_id, old('members', [])) ? 'checked' : '' }}>
                        ...
                        <p class="px-3 py-2 text-sm" style="color: var(--color-primary-500);">No other workspace members are available yet.</p>
                        @endif
                    </div>
                </div>
