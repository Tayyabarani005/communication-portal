    ...
    <div class="flex flex-col flex-1 overflow-hidden">
        ...
        <div class="flex items-center justify-between px-5 py-3 border-b flex-shrink-0" style="background: white; border-color: var(--color-border);">
            ...
            <div class="flex items-center gap-2">
                ...
                @can('update', $channel)
                <a href="{{ route('channels.edit', $channel) }}" class="p-2 rounded-lg transition-colors hover:bg-gray-100" style="color: var(--color-primary-500);" title="Edit Channel">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                ...
                @endcan
                @can('delete', $channel)
                <form method="POST" action="{{ route('channels.destroy', $channel) }}" onsubmit="return confirm('Are you sure you want to delete this channel? All messages and tasks will be permanently deleted. This cannot be undone.');" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="p-2 rounded-lg transition-colors hover:bg-red-50 text-red-600 hover:text-red-700" title="Delete Channel" style="background: none; border: none; cursor: pointer;">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
